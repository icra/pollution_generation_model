...
def run_pollutant_generation_model(
    pollutants: list[str],
    watershed: str,
    ignore_industries: bool = True,
    removal_rate_path: str | Path | None = None,
    edar_data_xlsx: str | Path | None = None,
):
    """
    Runs the pollutant generation model for a given watershed and list of pollutants.

    This function simulates pollutant loads generated by wastewater treatment plants (WWTPs) 
    and their discharge to rivers based on effluent characteristics, removal rates, and optional 
    industrial discharges. It returns a nested dictionary containing the modeled output for each 
    WWTP and associated discharge points.

    Parameters:
        pollutants (list[str]): List of pollutant names (e.g., chemical compounds) to be modeled.
        watershed (str): Name of the watershed or basin to model (e.g., "llobregat").
        ignore_industries (bool, optional): Whether to exclude industrial contributions from the model.
            Defaults to True.
        removal_rate_path (str | Path | None, optional): Path to an Excel file containing removal 
            rates and generation coefficients for the pollutants. If None, defaults are used.
        edar_data_xlsx (str | Path | None, optional): Path to an Excel file containing WWTP (EDAR) 
            data, including flow rates, configurations, and population served. If None, defaults are used.
...

            {
              "wwtp": {
                "edar_eu_code": {
                  ...
                  "dc_code": str,
                  "nom": str,
                  "population_real": float,
                  "configuration": list[str],  # e.g., ["P", "SN", "UF"]
                  "compounds_effluent": {
                    "q": float,               # Effluent flow in m³/day
                    "pollutant_1": float,     # Pollutant load in kg/day
                    ...
                  }
                },
                ...
